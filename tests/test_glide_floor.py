...
# 4. Режим сближения требует подтверждения ПО ВРЕМЕНИ: на стенде время до
#    контакта выдало 0.61 с там, где ничего не сближалось. Выдержка теперь
#    по монотонному времени, а не по числу кадров (не плывёт с FPS).
assert zn["CLOSING_CONFIRM_TIME_S"] >= 0.15, (
    "выдержка короче 0.15 с одиночный выброс не отсечёт")
assert "_vyderzhka_closing" in src and "_vyderzhka_gotova(" in src, (
    "режим сближения снова включается с первого кадра")

print("наклон только вниз, буст газа отвязан, режим сближения с выдержкой %.2f с"
      % zn["CLOSING_CONFIRM_TIME_S"])
