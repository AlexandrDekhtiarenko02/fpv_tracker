...
def template_match_locked(gray, pred_cx, pred_cy, flow_motion=0.0):
    ...
    try:
        ...
        # Радиус исключения вокруг пика привязан к размеру ЦЕЛИ, а не к
        # размеру карты откликов.
        #
        # Раньше он считался как 15% от карты, а карта растёт вместе с окном
        # поиска, которое растёт со скоростью цели:
        #     медленная цель, окно 14 -> карта ~29 px -> радиус 4
        #     быстрая цель,   окно 29 -> карта ~59 px -> радиус 8
        # Собственный пик шире 4 px, поэтому на медленных кадрах в «лучшего
        # конкурента» попадало ПЛЕЧО ТОГО ЖЕ ПИКА, а не другой объект. Это
        # объясняет и перекос срабатываний (45% на медленных против 9% на
        # быстрых), и то, почему уменьшение фона в шаблоне на эту долю никак
        # не повлияло: измерялся не фон, а собственный пик.
        #
        # Ширина пика определяется размером ЦЕЛИ: сдвиг больше её размера
        # разрушает совпадение. Цель занимает примерно 1/TEMPLATE_SCALE от
        # шаблона, отсюда и оценка. Верхняя граница нужна, чтобы на маленькой
        # карте осталось из чего считать фон.
        r_target = int(tmpl_w / max(TEMPLATE_SCALE, 1.0))
        r_cap = max(2, int(min(sm.shape) * 0.35))
        r = max(3, min(r_target, r_cap))
