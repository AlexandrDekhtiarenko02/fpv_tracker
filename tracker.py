...
# чем точнее слежение.
PITCH_SIGN = +1
# Крен и рыскание НЕ ЗАМЕРЕНЫ. Тангаж на этом борту оказался обратным
# «стандартному», поэтому принимать эти два на веру больше нельзя:
# tools/os_znaki.py --kren проверяет крен тем же способом по моторам.
# Рыскание по моторам не определить — направление зависит ещё и от того,
# куда крутятся винты, чего MSP_MOTOR не сообщает; его знак проверяется
# только в воздухе.
ROLL_SIGN = +1
YAW_SIGN = +1
