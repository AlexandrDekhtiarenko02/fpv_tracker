...
Четыре контрольные точки на ось (roll/pitch):
...
диагностики), а то, что все четыре точки СУЩЕСТВУЮТ, отличимы друг от
друга при trust_k < 1, и что at trust_k == 1 (слежение стабильно) вся
цепочка requested -> after_trust не меняет знак/масштаб произвольно.
"""
import os
import sys
...
print("\n=== 5. after_slew ограничен CMD_SLEW_MAX_STEP от предыдущей "
...
# отправленной команды (_slew_roll/_slew_pitch).
c2 = kadr(0.90, dx=200, dy=150)
if t.CMD_SLEW_ENABLED:
    shag_max = t.CMD_SLEW_MAX_STEP + 1e-6
    assert abs(c2["roll_after_slew"] - 1500) <= max(
        shag_max, abs(c2["roll_before_slew"] - 1500)), (
...
print("\nOK: requested/after_trust/before_slew/after_slew присутствуют, "
      "согласованы между собой и с существующими доверием/slew")
