...
def _nayti_pyatno(gray):
    ...
    try:
        ...
        # ПРИЦЕЛ ИМЕЕТ ПРИОРИТЕТ. Есть СВОЯ вершина под прицелом — выбор
        # окончен, берём прицел и ни с чем не сравниваем: пилот уже навёл.
        #
        # Проверяется именно вершина, а не превышение порога. Порога мало:
        # широкое размытие расплывает отклик соседнего предмета далеко за его
        # края, и его хвост под прицелом порог превышает — захват решал бы,
        # что цель под прицелом есть, хотя там только край чужого отклика.
        # У хвоста вершины нет, у предмета есть.
        if np.any(rast <= ACQ_SNAP_YADRO_LORES):
