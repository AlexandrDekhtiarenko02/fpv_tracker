...
def color_projection(shape):
    ...
    try:
        ...
        # Считаем ОДНИМ действием и на МАЛОЙ картинке — цветность вчетверо
        # меньше кадра. Раскрываем скобки заранее:
        #   128 + ((U-ou)*du + (V-ov)*dv) * k
        #     = U*(du*k) + V*(dv*k) + (128 - (ou*du + ov*dv)*k)
        # ЗАМЕРЕНО на малине: 5.85 мс прежним способом (арифметика в дробных
        # числах по БОЛЬШОЙ картинке) против 0.54 мс этим. Вдесятеро дешевле
        # при том же результате слежения.
        k = TRACK_ON_COLOR_SCALE
        p = cv2.addWeighted(chroma_u, du * k, chroma_v, dv * k,
                            128.0 - (ou * du + ov * dv) * k, dtype=cv2.CV_8U)
