...
#
# Документация Picamera2 подтверждает: DRM preview ВСЕГДА сохраняет
# аспект источника (letterbox/pillarbox), явного режима "залить экран
# без полей" в API нет. Источник 640x480 (4:3) на экране 720x480 (3:2) —
# эти поля математически неизбежны при сохранении пропорций. Единственный
# способ убрать их совсем — просить окно на всю ширину активного режима
# (PREVIEW_W=ACTIVE_MODE_W=720) и посмотреть эмпирически на стенде, что
# в этом случае сделает скейлер: либо картинка растянется по горизонтали
# без явных чёрных полей (и на аналоговом NTSC-выходе это может выглядеть
# корректно за счёт неквадратных пикселей самого стандарта, либо будет
# заметно искажение/обрезка сверху-снизу) — это ПРОВЕРЯЕТСЯ СТЕНДОМ,
# не выводится теоретически.
ACTIVE_MODE_W = 720
