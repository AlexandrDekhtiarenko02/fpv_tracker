...
закон, выведенный из данных, обязан считаться из величин, которые на боевом
...
print("    боевых %d, только для кампании %d — все на месте" % (len(prod), len(camp)))

print("\n=== 2. Списки не пересекаются ===")
both = set(prod) & set(camp)
assert not both, "величина числится и боевой, и кампанийной: %s" % both
...
for need, why in (("ground_speed_ms", "путевая скорость без GPS"),
    ...
    assert need in prod, "%s (%s) обязана быть доступна на боевом борту" % (need, why)
    print("    %-16s %s" % (need, why))

print("\nOK: закон можно строить только из того, что есть на боевом борту")
