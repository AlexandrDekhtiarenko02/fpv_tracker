...
print("\n=== 5. Спрашиваем только про то, что на плате есть ===")
bez_maga = MASK & ~(1 << 2)
d = _kak_slovar(_tr(sensor_mask=bez_maga, **{k: v for k, v in
                                             _vse_podnyalos().items()
                                             if k != "sensor_mask"}))
print("    строки:", sorted(d))
assert "mag" not in d, (
    "борт без магнитометра вечно висел бы с ошибкой по отсутствующей железке")
assert "gyro" in d and "baro" in d

print("\n=== 6. Не ответил в срок — назван поимённо ===")
...
print("\n=== 7. Всё поднялось — отчёт уходит; отказ остаётся ===")
