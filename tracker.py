...
TRUST_PSR_FULL = 6.0
# Однозначность пика, ниже которой доверия НЕТ. При psr <= 1 пик и его
# ближайший сосед сравнимы по силе — матч в этот момент угадывает не тот
# кандидат так же часто, как тот. Раньше формула была psr / TRUST_PSR_FULL,
# и psr=1.06 давал часть 0.18: контур продолжал крутить руль почти на
# полную. Замер по zahvat14 сессии 12:08 — образец: psr падал 5.55 → 3.58
# → 2.35 → 1.40 → 1.06, а doverie в CSV держалось ≈ 0.99–1.00. Линейный
# коридор [LOW..FULL] лечит это, оставляя ровный ход неизменным (psr=5.7
# на спокойных заходах даёт часть 0.94, как раньше 0.95).
TRUST_PSR_LOW = 1.0
...
def update_control_from_target():
    ...
    if TRUST_ENABLED:
        ...
        if _psr is not None and _psr > 0.0:
            # Линейный коридор [LOW..FULL]: psr ниже LOW = ноль доверия,
            # выше FULL = полное. Прежняя formula psr/FULL давала при
            # psr=1 всего 0.17 — контур почти на полную мощность, хотя
            # матч в этот момент угадывает не хуже случайного соседа.
            _diap = max(1e-6, TRUST_PSR_FULL - TRUST_PSR_LOW)
            _psr_part = (float(_psr) - TRUST_PSR_LOW) / _diap
            if _psr_part < 0.0:
                _psr_part = 0.0
            elif _psr_part > 1.0:
                _psr_part = 1.0
            chasti.append(_psr_part)
