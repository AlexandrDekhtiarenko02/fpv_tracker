...
print("\n=== 6. Стороны втягиваются ЗАОДНО с оборотом ===")


def pustyh_seredin(kadr, r):
    """Сколько середин сторон пусты.

    В покое квадрат стоит ровно, значит середины сторон — точки строго
...
# В начале хода квадрат сплошной.
tselyh = pustyh_seredin(snyat(0.0), t.METKA_R_START)
print("    в начале хода:  пустых середин %d из 4" % tselyh)
assert tselyh == 0, "квадрат разорван с самого начала — раскрытие не с нуля"

# К концу того же хода — уголки. Отдельной фазы больше нет.
f = snyat(1.0)
pusto = pustyh_seredin(f, t.METKA_R_END)
print("    в конце хода:   пустых середин %d из 4" % pusto)
assert pusto == 4, (
    "к концу оборота середины сторон закрашены: раскрытие не успело, значит "
    "оно идёт не одним ходом с оборотом")

# Углы на месте — иначе метка просто исчезла.
ys, xs = np.nonzero(f[:, :, 0] > 200)
assert len(xs) > 0, "метка исчезла целиком"
r_ugly = float(np.hypot(xs - 160.0, ys - 120.0).max())
print("    углы на радиусе %.1f px (метка %g)" % (r_ugly, t.METKA_R_END))
assert abs(r_ugly - t.METKA_R_END) <= 2.0, "углы уехали с радиуса метки"

assert 0.0 < t.METKA_UGOL_DOLYA < 0.5, (
    "доля стороны %g: при 0.5 квадрат сплошной, при 0 углов не останется"
    % t.METKA_UGOL_DOLYA)
assert not hasattr(t, "METKA_UGLY_S"), (
    "осталась отдельная длительность раскрытия: движение снова разъедется "
    "на две фазы")

print("\nOK: лупа меньше, метка одним ходом крутится, поджимается и раскрывается")
