...
# чем точнее слежение.
PITCH_SIGN = +1
# Крен и рыскание проверяются тем же tools/os_znaki.py, шаги 3 и 4. Тангаж
# на этом борту оказался обратным «стандартному», поэтому принимать эти два
# на веру нельзя.
#
# Рыскание определяется по моторам не полностью: диагональ показывает знак
# команды в микшере, а сторону поворота задаёт ещё и направление вращения
# винтов. Его полётник сообщает сам — yaw_motors_reversed в MSP_MIXER_CONFIG,
# и вместе с диагональю этого хватает.
ROLL_SIGN = +1
YAW_SIGN = +1
