...
MATCH_AMBIGUITY_GUARD = False
# Вес матча в слиянии с потоком.
#
# ВЫВОД ИЗ ТРЁХ ЗАМЕРОВ НА БОРТУ. Защита от неоднозначного матча давала
# выигрыш, но, как выяснилось, не тем, что распознавала конкурентов — а тем,
# что часто СНИЖАЛА доверие к матчу. Средний вес и средний захват легли на
# одну монотонную зависимость:
#     вес 0.220 (защиты нет)      -> захват  6.6 с
#     вес 0.164 (защита 28%)      -> захват  9.3 с
#     вес 0.130 (защита 45%)      -> захват 18.5 с
# Причём точка 0.164 получена при БОЛЕЕ ТОЧНОМ определении конкурентов — и
# оказалась хуже. То есть польза шла от величины веса, а не от распознавания.
#
# Поэтому вся машинерия с конкурентами отключена, а вес просто понижен.
# 0.10 лежит ниже лучшей измеренной точки.
#
# ПРОВЕРЯЕМОЕ ПРЕДСКАЗАНИЕ: средний захват не меньше 18.5 с.
#
# РИСК: при слишком малом весе ведёт почти один оптический поток, а он копит
# дрейф. Тогда рамка будет медленно сползать с цели на длинных захватах —
# смотреть на длинные заходы, а не только на среднее.
MATCH_WEIGHT = 0.10
