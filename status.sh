...
# Ищем лог ПО НОМЕРУ ПРОЦЕССА, а не «самый свежий по времени».
# Выбор по времени промахивается: при перезапуске старый процесс дописывает
# разбор вылета уже после того, как новый создал свой файл, и его лог
# оказывается новее. Номер процесса такой двусмысленности не имеет.
EVT=""
for f in $(ls -t flight_logs/*.events.log 2>/dev/null); do
    if grep -q "session start pid=${MAINPID}\b" "$f" 2>/dev/null; then
        EVT="$f"; break
    fi
done
if [ -z "${EVT:-}" ]; then
    if ls flight_logs/*.events.log >/dev/null 2>&1; then
        echo "  Лог процесса $MAINPID ещё не создан — служба только что"
        echo "  стартовала. Подожди пару секунд и повтори."
        exit 1
    fi
