...
#
# ПОДОБРАНО ПЕРЕБОРОМ ПО ЗАПИСЯМ — впервые за всё время не на глаз. Семь
# записей со стенда, 2600 кадров, мерило: доля кадров, когда рамка стоит на
# НЕПОДВИЖНЫХ пикселях (камера на стенде неподвижна, значит движется только
# рука с целью, и рамка на неподвижном пикселе — это рамка на фоне):
#
#     0.55 (было) -> 6.1%
#     0.42        -> 3.2%
#     0.30        -> 3.1%
...
#     0.12        -> 2.6%
#
# Механизм понятен и не случаен: предсказание потока САМО сползает на фон, а
# сильный штраф заставлял совпадение держаться возле сползшего предсказания.
# Ослабление штрафа позволяет матчу вернуться на цель.
#
# Главный выигрыш — на записи 175812, где рамка уходила на перфорированную
# стену: 17.5% -> 1.7%.
#
# ОГОВОРКА ЧЕСТНАЯ: все семь записей сняты со стенда неподвижной камерой и с
# медленным движением. В полёте камера движется сама, и предсказание потока
# там значит больше. Проверять на первом же живом заходе.
DIST_PENALTY = 0.20
MAX_LOCK_STEP = 32
