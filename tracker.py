...
#
# ЗАМЕРЕНО ЗАНОВО ПО ПОЛЁТАМ С УПРАВЛЕНИЕМ, 9 сентября 2026.
#
# Прежний замер (2.02) снят целиком в режиме наблюдения: рулил пилот и сам
# держал цель в центре, то есть компенсировал руками. Тот комментарий это и
# оговаривал — «нужна ли она замкнутому контуру и в каком размере, покажет
# первый полёт с управлением». Полёты состоялись, и ответ другой.
#
# Как мерялось. Берутся участки, где тангаж едет монотонно хотя бы на 5°,
# не меньше 10 кадров и не быстрее 2.5°/кадр, и считается отношение сдвига
# рамки к изменению угла. Ограничение по темпу обязательно: на рывках (30-110°
# за 2-9 кадров) трекер теряет цель, рамка замирает, и отношение выходит около
# нуля — это замер срыва слежения, а не геометрии. Именно такие участки и
# тянули прежнюю оценку вниз: по ним медиана 1.71, по плавным — 6.67.
#
# Точность честно ограничена: половины выборки дали 5.20 и 8.58, расхождение
# 51%. Значит точное число этими данными не определяется. НО обе половины лежат
# выше 2.0 в 2.6-4.3 раза — вот это и есть надёжный вывод.
#
# Выбрано 5.0 — ниже обеих половин. Недобор безопаснее перебора: геометрия
# кадра даёт 11.6 px/°, и компенсация ВЫШЕ этого превращает обратную связь в
# положительную (прицел уезжает дальше цели, нос валится всё круче). 5.0
# оставляет двойной запас до этой границы.
#
# Во что обходилось прежнее значение — видно в логах от 9 сентября: за 12°
# наклона цель уезжала на 135 px, компенсация давала 25, остальные 110 px
# контур принимал за движение цели. При P_GAIN_PITCH = 3.5 это 385 PWM, то
# есть упор. Отсюда и рывки носом.
PIXELS_PER_PITCH_DEG = 5.0
...
# Предохранитель от абсурда, а НЕ рабочий режим: пока он срабатывает,
# компенсация перестаёт следить за наклоном и контур снова принимает
# собственный тангаж за движение цели.
#
# 220 px рассчитано под рабочий диапазон: при 5.0 px/° и пикировании до 40°
# компенсация доходит до 200 px и в предел не упирается. Прежние 100 px
# срабатывали уже на 20°, то есть на всех наблюдавшихся заходах (30-37°) —
# компенсация стояла колом ровно там, где нужнее всего.
MAX_PITCH_COMP_PX = 220.0
