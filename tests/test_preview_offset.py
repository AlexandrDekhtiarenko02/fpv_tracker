...
История диагностики:
  ...
  - kmsprint -m + прямой осмотр слоёв по SSH дали настоящую причину:
    ...
    вовсе.
  - Первая версия фикса (PREVIEW_W/H=640x480, PREVIEW_X=0) полосу сверху
    убрала, но стенд тут же показал новый симптом: активный режим шире
    окна (ACTIVE_MODE_W=720 vs PREVIEW_W=640), и при PREVIEW_X=0 вся
    разница (80px) уходила ЦЕЛИКОМ в правый край — "огромная полоса
    справа", картинка визуально уехала влево. PREVIEW_X теперь
    центрирует окно: та же неиспользуемая площадь, но поровну по обе
    стороны.

Фикс: PREVIEW_W/PREVIEW_H = точный нативный размер камеры (MAIN_W/MAIN_H)
— источник вписывается 1:1, без масштабирования и без letterbox вообще.
PREVIEW_X = (ACTIVE_MODE_W - PREVIEW_W) // 2 — центрирует по ширине.
...
print("\n=== 2. PREVIEW_X центрирует окно в активном режиме, PREVIEW_Y=0 ===")
...
print("\nOK: PREVIEW_W/H приведены к нативному размеру камеры (без "
      ...
      "480-строчного режима), PREVIEW_X центрирует окно по ширине активного "
      "режима, диагностика top-strip сохранена для будущей перепроверки")
