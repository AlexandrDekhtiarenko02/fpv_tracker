...
полей вокруг картинки (стендовые правки после b1fb124/0759d88/93fd335).
...
История диагностики:
  ...
  - Фикс PREVIEW_H=MAIN_H=480 (высота ровно как активный режим) убрал
    полосу СВЕРХУ полностью — это подтверждено и с тех пор не менялось.
  - PREVIEW_W=MAIN_W=640 (первая версия, без масштабирования вовсе) убрал
    letterbox, но по ширине активный режим (ACTIVE_MODE_W=720) шире окна
    — при PREVIEW_X=0 вся разница (80px) уходила ЦЕЛИКОМ в правый край
    ("огромная полоса справа"). Центрирование (PREVIEW_X=40) сделало
    поля симметричными (по 40px), но не убрало их — see below.
  - Документация Picamera2 подтвердила: DRM preview ВСЕГДА сохраняет
    аспект источника (letterbox/pillarbox), явного режима "залить экран
    без полей" в API нет. Источник 4:3 на экране 3:2 (720x480) — поля
    математически неизбежны при сохранении пропорций БЕЗ растяжения.
  - Текущий эксперимент: PREVIEW_W=ACTIVE_MODE_W=720 (запросить окно на
    всю ширину активного режима) — что именно сделает скейлер в этом
    случае (растянет без искажения на глаз за счёт неквадратных пикселей
    NTSC, либо всё равно добавит letterbox сверху/снизу, либо обрежет)
    ПРОВЕРЯЕТСЯ СТЕНДОМ, не выводится теоретически — отсюда PREVIEW_W
    здесь = ACTIVE_MODE_W, а не MAIN_W.

Инвариант, который остаётся верным независимо от исхода эксперимента:
PREVIEW_H = MAIN_H (480, закрывает полосу сверху — это решённая часть),
PREVIEW_X центрирует окно в активном режиме, офсет изолирован в display
path и не течёт в геометрию трекера.
...
print("\n=== 2. PREVIEW_W — либо нативный размер камеры, либо явно на всю "
      "ширину активного режима (текущий эксперимент) ===")
assert pw in (main_w, active_mode_w), (
    "PREVIEW_W=%d — не нативный размер камеры (%d, гарантированно без "
    "letterbox) и не ширина активного режима (%d, эксперимент "
    "'растянуть на весь экран'). Любое другое значение непроверено ни "
    "теоретически, ни стендом" % (pw, main_w, active_mode_w))
if pw == active_mode_w:
    print("    PREVIEW_W=ACTIVE_MODE_W=%d — окно на всю ширину экрана, "
          "исход (растяжение/обрезка/новый letterbox) подтверждается "
          "стендом" % pw)
else:
    print("    PREVIEW_W=MAIN_W=%d — без масштабирования по ширине, "
          "пилларбокс неизбежен и принят как есть" % pw)
...
print("\nOK: PREVIEW_H держит закрытой полосу сверху (решённая часть), "
      "PREVIEW_W — либо нативный размер камеры, либо на всю ширину "
      "активного режима (текущий стендовый эксперимент), PREVIEW_X "
      "центрирует, диагностика top-strip сохранена")
