...
def _nayti_pyatno(gray):
    ...
    Возвращает (x, y) в координатах lores либо None, если в зоне нет ничего
    выраженного — тогда захват идёт ровно туда, куда навёл пилот.
    """
    ...
    try:
        ...
        # друга давали одну область площадью 699 с центром в пустоте между
        # ними.
        #
        # Локальный максимум такой беды не имеет: у каждого предмета своя
        ...
        if not np.any(vnutri):
            return None
        # ВЫБИРАЕТ РАССТОЯНИЕ, а не выраженность: см. пояснение выше.
