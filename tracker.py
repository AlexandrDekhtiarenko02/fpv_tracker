...
# --- ВЫДЕРЖКА КАМЕРЫ ---
# Наблюдение из полёта: быстро ведёшь объект — лок прыгает на фон, медленно —
# держится. Причина не в алгоритме, а в СМАЗЕ.
#
# Выдержка ограничивалась 33 мс — это длительность целого кадра при 30 к/с.
# Цель, смещающаяся на 12 пикселей за кадр, за время такой выдержки
# размазывается на все 12 пикселей, а сама цель размером 8-16 пикселей. То
# есть на скорости цель превращается в полосу, тогда как неподвижный фон
# остаётся РЕЗКИМ — и честно выигрывает сравнение по шаблону.
#
# При 8 мс тот же смаз втрое меньше и цель остаётся узнаваемой.
#
# ЧЕМ ПЛАТИМ: света вчетверо меньше, поэтому усиление поднимается во столько
# же и растёт шум. Для нормированной корреляции это выгодный размен: шум
# некоррелирован и почти не влияет на совпадение, а смаз разрушает саму
# структуру, по которой идёт сравнение.
#
# ВАЖНО: в помещении автоэкспозиция и выбирает эти 20-33 мс. На улице днём
# она сама даёт 1-5 мс, поэтому там отказ должен быть заметно слабее — то
# есть офисные испытания преувеличивают проблему.
# ЗАМЕРЕНО: жёсткие 8000 мкс оказались вредны. В помещении автоэкспозиция
# стояла на 32939 мкс при усилении 4.0; чтобы вернуть свет при 8000, усиление
# должно было вырасти до 16.5, а потолок 8.0 — картинка стала ВДВОЕ темнее с
# удвоенным шумом. Мелкие дальние цели от этого страдают сильнее всего: им
# нечем выделиться из шума. score 0.693 -> 0.617, провалов 8%% -> 16%%.
#
# Поэтому выдержка укорачивается не до фиксированного числа, а РОВНО
# НАСТОЛЬКО, насколько усиление способно вернуть свет. Яркость сохраняется, а
# смаз уменьшается настолько, насколько это бесплатно.
#   в помещении (33 мс, усиление 4) -> около 16 мс, смаз вдвое меньше
#   на улице днём (2 мс, усиление 1) -> 8 мс не нужны, остаётся 2 мс
MIN_EXPOSURE_US = 8000
# Потолок усиления. Выше — шум начинает съедать мелкие цели.
...
# --- ОКНО ПОИСКА ПРОПОРЦИОНАЛЬНО РАЗМЕРУ ЦЕЛИ ---
# Наблюдение из полёта: пока объект БОЛЬШЕ или равен пятнам на фоне, лок на
# фон не перепрыгивает; как только объект мельче — прыгает.
#
# Объяснение: окно поиска задано в АБСОЛЮТНЫХ пикселях, безотносительно
# размера цели. Значит в одно и то же окно для крупной цели помещается пара
# объектов её размера, а для мелкой — десятки пятен её размера, и любое
# может выиграть сравнение. Число конкурентов определяется ОТНОШЕНИЕМ окна к
# цели, а сейчас это отношение гуляет в разы.
#
# Здесь окно складывается из двух частей: доли размера цели и запаса на её
# движение. Коэффициенты подобраны так, чтобы для типичной цели 16 px
# получались НЫНЕШНИЕ значения (14 в покое, 36 на полной скорости) — то есть
# для типового случая ничего не меняется, а расходятся только края.
SEARCH_MARGIN_BY_SIZE = True
SEARCH_MARGIN_SIZE_K = 0.9        # доля размера цели
SEARCH_MARGIN_MOTION_K = 2.5      # запас на смещение за кадр
# ЗАМЕРЕНО: при коэффициенте 1.8 и пределе 8 окно у мелкой цели сжималось до
# ~10 px. Качество матча стало лучшим за все опыты (score 0.723, провалов
# 11%) — то есть гипотеза про число конкурентов верна. Но захваты укоротились
# с 18.5 до 10.1 с: быстрое движение выбрасывало цель ЗА ПРЕДЕЛЫ окна, и
# прыжки на фон сменились потерями. Поэтому запас на движение увеличен, а
# нижний предел поднят: сохранить выигрыш в качестве, вернув длину захвата.
# Абсолютный нижний предел: у совсем мелкой цели окно не должно схлопнуться
# настолько, чтобы быстрое движение выбрасывало её за пределы окна — это
# дало бы потерю вместо прыжка на фон.
SEARCH_MARGIN_FLOOR = 12 * TRACK_SCALE
...
def template_match_locked(gray, pred_cx, pred_cy, flow_motion=0.0):
    ...
    if SEARCH_MARGIN_BY_SIZE:
        # Размер цели: шаблон в TEMPLATE_SCALE раз больше неё.
        tgt = max(4.0, float(tmpl_w) / max(TEMPLATE_SCALE, 1.0))
        m = tgt * SEARCH_MARGIN_SIZE_K + flow_motion * SEARCH_MARGIN_MOTION_K
        margin = int(round(max(SEARCH_MARGIN_FLOOR, min(SEARCH_MARGIN_MAX, m))))
    elif ADAPTIVE_SEARCH_MARGIN:
...
def main():
    ...
    try:
        ...
        exp_auto = int(md.get("ExposureTime", 8000))
        gain_auto = float(md.get("AnalogueGain", 1.0))
        # Насколько короткую выдержку усиление ещё может компенсировать.
        affordable = exp_auto * gain_auto / MAX_ANALOGUE_GAIN
        exp = int(max(MIN_EXPOSURE_US, affordable))
        exp = min(exp, exp_auto)          # длиннее автоматической не делаем
        gain = gain_auto * (float(exp_auto) / float(max(exp, 1)))
        gain = max(1.0, min(MAX_ANALOGUE_GAIN, gain))
        try:
            print("[tracker] выдержка %d -> %d мкс, усиление %.2f -> %.2f"
                  % (exp_auto, exp, gain_auto, gain), flush=True)
            flight_log.event("КАМЕРА выдержка %d -> %d мкс | усиление %.2f -> %.2f"
                             % (exp_auto, exp, gain_auto, gain))
        except Exception:
            pass
        ctrl = {
            "AeEnable": False,
