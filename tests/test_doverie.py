...
print("\n=== 2б. Медленное ухудшение НЕ становится новой нормой ===")
# Здесь была ошибка, из-за которой доверие показывало 1.00 даже в самых
# рваных заходах: самокалибровка сравнивает величину с её же недавним
# уровнем, а при медленном ухудшении уровень едет следом.
sbros()
for _ in range(30):
    kadr(0.80, psr=6.0)
# psr сползает вдвое за сотню кадров — медленно, но это настоящая беда.
for i in range(100):
    psr_tek = 6.0 - 3.0 * (i / 99.0)
    polzet = kadr(0.80, psr=psr_tek)
print("    psr сполз с 6.0 до 3.0 за 100 кадров -> тангаж %d" % polzet)
sbros()
for _ in range(30):
    rovno = kadr(0.80, psr=6.0)
print("    при psr 6.0 всё время           -> тангаж %d" % rovno)
assert abs(polzet - 1500) < abs(rovno - 1500) * 0.85, (
    "медленное сползание psr не урезало усиление: самокалибровка приспособилась "
    "к деградации вместо того, чтобы о ней доложить")
